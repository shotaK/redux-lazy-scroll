...
//defining schema for posts table
...
postsModel.seed = function(){
	...
	for(var i=0; i<52; i++){
		...
		dataToInsert.push(postObject);
	}
	
	Post.collection.insert(dataToInsert, function(err, post) {
		if(err){
			console.log(err);
		} 
		else{
			console.log('Posts table has been populated successfully.');
		}	
	});
...
postsModel.get = function(skip, limit){
	...
	Post.find(function(err, dbPost) {
		if (err){
			results.reject(err);
		} 
		
		results.resolve(dbPost);
